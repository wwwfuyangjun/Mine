$(function() {
	...
	//百度分享初始化位置
	$('#share').css('top', (getInner().height - parseInt(getStyle($('#share').first(), 'height'))) / 2 + getScroll().top + 'px');
	$(window).bind('scroll', function() {
		setTimeout(function() {
			$('#share').animate({
				attr: 'y',
				target: (getInner().height - parseInt(getStyle($('#share').first(), 'height'))) / 2 + getScroll().top
			});
		}, 100);
	...
	function check_pass() {
		...
		if(value_length >= 6 && value_length <= 20) {
			...
			$('#reg .info_pass .q1').html('○').css('color', '#666666');
		}
		//第二个条件验证，大小写字母、数字和非空格字符
		if(value_length > 0 && !/\s/.test(value)) {
			$('#reg .info_pass .q2').html('●').css('color', 'green');
		} else {
			$('#reg .info_pass .q2').html('○').css('color', '#666666');
		}
		//第三个条件验证，大、小写字母、数字、非空字符，2种以上
		if(/[\d]/.test(value)) {
			code_length++;
		}
		if(/[a-z]/.test(value)) {
		...
		if(value_length >= 6 && value_length <= 20 && !/\s/.test(value) && code_length >= 2) {
			if(value_length >= 10 && code_length >= 3) {
				...
				$('#reg .info_pass .s3').css('color', '#CCCCCC');
				$('#reg .info_pass .s4').html('中').css('color', '#f60');
			} else if(value_length >= 1) {
				$('#reg .info_pass .s1').css('color', 'maroon');
				$('#reg .info_pass .s2').css('color', '#CCCCCC');
				$('#reg .info_pass .s3').css('color', '#CCCCCC');
				$('#reg .info_pass .s4').html('低').css('color', 'maroon');
			}
			return true;
		} else {
			$('#reg .info_pass .s1').css('color', '#CCCCCC');
			$('#reg .info_pass .s2').css('color', '#CCCCCC');
			$('#reg .info_pass .s3').css('color', '#CCCCCC');
	...
	$('form').form('email').bind('keyup', function(event) {
		...
		$('#reg .all_email li').css('color', '#666666');
		if(event.keyCode == 40) {
	...
	$('#banner ul li').eq(0).css('color', '#333333');
